...
- [ ] 화면 설계 (와이어프레임 또는 화면 목록 정의)
  - [ ] SCR-01. 홈 / 입력 화면: 자유 입력 텍스트필드, 입력 힌트(placeholder), 보정 시작 버튼
  - [ ] SCR-02. 확신도 마킹 화면: 입력 텍스트에서 불확실 구간 선택/태그 처리 UI
  - [ ] SCR-03. 추론 중 화면: 로딩 인디케이터, "기억을 분석하고 있습니다..." 안내 문구
  - [ ] SCR-04. 보정 결과 화면: 원문 vs 보정 결과 대조 카드, 보정 이유 설명, 확률 표시
  - [ ] SCR-05. 출처/근거 화면: 뉴스 링크·영상 목록, 추측 항목은 출처 없이 확률만 표시
  - [ ] SCR-06. 이력 목록 화면: 과거 보정 이력 리스트 (날짜, 입력 원문 요약, 보정 결과 요약)
  - [ ] SCR-07. 이력 상세 화면: 선택한 이력의 전체 입력 원문, 보정 결과, 출처 재표시
