...
- [x] Gugeo Engine API 인터페이스 설계 (Google Gemini API 기반)
  ...
  - [x] GE-04. 입력 파싱 인터페이스 정의 (GugeoRequest) → `domain/GugeoEngine.kt`
  - [x] GE-05. 응답 파싱 인터페이스 정의 (GugeoResponse, GugeoEngine) → `domain/GugeoEngine.kt`
...
- [x] FR-06. 보정 이력 저장 및 조회 구현 (Room DB 연동)

## 페이즈 06. 인프라/기타

- [x] 프로젝트 아키텍처 설계 (data, domain, ui 레이어 분리)
- [x] 의존성 최종 확인 및 빌드 검증 (assembleDebug 성공)

## 페이즈 07. UX 고도화 및 대화형 인터페이스 전환

- [x] NFR-02-C. 채팅형 인터페이스 레이아웃 구현 (입력창 하단 고정, 상단 메시지 히스토리)
- [x] Design-01. Gemini 웹 스타일 UI 테마 적용 (카드형 메시지 버블, 부드러운 애니메이션)
- [x] FR-01-B. 모호한 입력을 유도하는 제안용 칩(Suggestion Chips) UI 추가
- [x] FR-02-B. 텍스트 어절 단위 마킹 UI 구현 (채팅 흐름 내 통합)
- [x] FR-05-B. 출처/근거 카드 URL 연결 기능 구현 (Intent 연동)
- [x] SCR-03-B. 로딩 화면 단계별 안내 문구 애니메이션 구현 (채팅창 내 로딩 상태 표시)
- [x] UX-01. 대화 흐름 기반의 연속 보정 기능 (결과 확인 후 추가 질문 가능하도록 구조 개선)
- [x] NFR-04-B. API 키 설정 화면을 설정 다이얼로그 또는 사이드바로 이동 (대화 집중도 향상)

## 페이즈 08. 설정 화면 추가
- [x] 설정 버튼 클릭 시 팝업이 아니라 별도 화면으로 이동

## 페이즈 09. 채팅 목록
- [ ] 기억 보정하기 실행하면 > 과거 채팅 질문을 수정하는게 아니라, 채팅을 이어서 진행 할 수 있도록 수정

## 페이즈 10, 앱 최초 실행 버그 수정
- [ ] 앱 최초 실행 > 더미 데이터 채팅 목록 15건 표시
